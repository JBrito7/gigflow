...
// Message for error or success EDITING the status from the order
$error = '';
$success = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!in_array($status, ['pending', 'paid', 'canceled'])) {
    $error = "Status selecionado inválido!";
  } else {
    $sql = "UPDATE purchases SET status='$status' WHERE id = $order_id";

    // Display feedback message
    if ($conn->query($sql) === TRUE) {
      $success = "Status actualizado com sucesso!";
      $order['status'] = $status;
    } else {
    ...
    <section id="dashboard">
      <div class="container py-5 mt-5">
        ...
        <form method="POST">
          ...
          <button type="submit" class="btn-color">Atualizar Status</button>
